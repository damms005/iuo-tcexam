...
/**
 * TCExam adds time relative to start time of the exam. However, practily often, we actually want to add
 * time relative to current time (i.e. at the very moment we are adding the time) so that the student/exam-taker
 * can continue exam immediately. A practical scenario: A 30 mins exam is ongoing, power outage happens 12 mins into
 * the exam, power restored 45 mins later. In current TCExam, there is no easy way of making them all continue from where
 * they left off. Hacky way-out with current TCExam is to keep
 * adding +5mins until start_time is brought to current time)
 *
 * @param [type] $testuser_id
 * @param [type] $relative_to_current_time
 * @return void
 */
function F_getTestNewStartTime($testuser_id, $relative_to_current_time, $entend_time = K_EXTEND_TIME_MINUTES)
{
    if ($relative_to_current_time) {
        //the normal thing does strtotime(test_creation_time). Test creation time example format is: "2018-01-31 13:13:30"
        return date(K_TIMESTAMP_FORMAT, time() + ($entend_time * K_SECONDS_IN_MINUTE));
    } else {
        return date(K_TIMESTAMP_FORMAT, F_getTestStartTime($testuser_id) + ($entend_time * K_SECONDS_IN_MINUTE));
    }
}
