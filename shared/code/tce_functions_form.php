 ...
 * @return string
 */
function getFormRowTextBox($field_name, $name, $description = '', $value = '', $disabled = false, $prefix = '', $addEditor = false, $form_id = '')
{
    require_once dirname(__FILE__) . '/../config/tce_config.php';
    global $l;
    if (strlen($description) == 0) {
    ...
    if ($addEditor) {
        $str .= '<br />' . K_NEWLINE;
        $str .= tcecodeEditorTagButtons('form_testeditor', $field_name);
        $str .= '<br />' . K_NEWLINE;
    }
    $str .= '</span>' . K_NEWLINE;
    $str .= '</div>' . K_NEWLINE;
