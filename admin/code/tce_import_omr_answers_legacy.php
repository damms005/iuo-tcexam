...
if (isset($menu_mode) and ($menu_mode == 'upload') and ($user_id > 0) and !empty($_FILES)) {
    ...
    if ($omr_testdata === false) {
        ...
        if( !there_is_enough_units_for_marking( $num_answer_sheets , count( array_keys( $uploaded_data ) ) ) ){
            endMarkingSessionWithError( $_POST['job_id'] , "Units exhaustion error: there are {$num_answer_sheets} answer sheets to mark. But you only have " . get_marking_units_left() . " units left");
        }
        ...
        if( substract_marking_units( $num_answer_sheets ) ) {
            // import answers
