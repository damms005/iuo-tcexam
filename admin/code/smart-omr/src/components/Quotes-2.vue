...
export default {  
  ...
  data:function(){
    return {
      ...
      ongoingTimeout: 0,
      progressbar: null,
      _incrementPerSecond: 0,
      timeoutEnsreCompleteAnimation: null
    }
  },
  computed: {
  },
  ...
  methods:{
    displayNextQuote:function(){
      if( this.available_quotes.length > 0 ) {
        ...
        this._incrementPerSecond = 100 /quoteDisplayTimeInSecs;
        var incrementPerSecond = this._incrementPerSecond;
        ...
        setTimeout(function(){
          ...
          // //check if animation is at one
          // //if at one, the display next quote
          // //else, wait for some time and do again

          // //tarry until the progreenar has animated to 100
          // ( function( callback ) {
          //   console.log(`we now at ${vm.progressbar.value()}`);
          //   if ( vm.progressbar.value() > 0 && vm.progressbar.value() >= 1) {
          //   } else {
    ...
    incrementQuoteWidth:function(increment){
        ...
        this.ongoingTimeout = setTimeout(function(){
          vm.incrementQuoteWidth( nextIncrement );
        }, 1000 )

        if(){
          if(this.timeoutEnsreCompleteAnimation){
          clearTimeout(this.timeoutEnsreCompleteAnimation);
          }

          this.timeoutEnsreCompleteAnimation = setTimeout(function(){vm.displayNextQuote();} , 3000 );
        }
    },
